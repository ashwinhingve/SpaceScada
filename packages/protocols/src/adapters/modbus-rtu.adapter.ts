 ...
 *
 * Production-ready implementation of Modbus RTU over serial port.
 * Supports RS-232, RS-485, and USB-to-Serial connections.
 */
...
export interface ModbusRTUConfig extends Omit<ConnectionConfig, 'port'> {
  port: string; // Serial port path (e.g., "/dev/ttyUSB0", "COM3")
  timeout?: number;
  options?: {
    baudRate?: number;
    dataBits?: 7 | 8;
...
export class ModbusRTUAdapter extends BaseProtocolAdapter {
  ...
  protected async doConnect(config: ConnectionConfig): Promise<void> {
    const rtuConfig = config as unknown as ModbusRTUConfig;

    if (!rtuConfig.port) {
      throw new Error('Modbus RTU requires port parameter (serial port path)');
    }

    const timeout = rtuConfig.timeout || 5000;
    const baudRate = rtuConfig.options?.baudRate || 9600;
    const dataBits = rtuConfig.options?.dataBits || 8;
    const stopBits = rtuConfig.options?.stopBits || 1;
    const parity = rtuConfig.options?.parity || 'none';

    this.defaultSlaveId = rtuConfig.options?.defaultSlaveId || 1;

    // Set timeout
    this.client.setTimeout(timeout);

    // Connect to serial port
    await this.client.connectRTUBuffered(rtuConfig.port, {
      baudRate,
      dataBits,
      stopBits,
      parity,
    ...
    console.log(
      `[ModbusRTU] Connected to ${rtuConfig.port} (${baudRate} baud, ${dataBits}${parity[0].toUpperCase()}${stopBits}), Slave ID: ${this.defaultSlaveId}`
    );
  }
  ...
  protected async doDisconnect(): Promise<void> {
    ...
    this.client.close(() => {
      console.log('[ModbusRTU] Disconnected');
    });
  }
  ...
  protected async doRead(address: string): Promise<DataPoint> {
    ...
    try {
      ...
      console.error(`[ModbusRTU] Read error for ${address}:`, error);
      return {
        address,
        name: this.generateTagName(parsedAddress),
        value: null,
  ...
  protected async doWrite(address: string, value: unknown): Promise<void> {
    ...
    try {
      ...
      console.log(`[ModbusRTU] Write successful: ${address} = ${value}`);
    } catch (error) {
      console.error(`[ModbusRTU] Write error for ${address}:`, error);
      throw error;
    } finally {
      // Reset to default slave ID
      if (parsedAddress.slaveId !== this.defaultSlaveId) {
  ...
  async discoverDevices(): Promise<DeviceDiscovery[]> {
    ...
    for (let slaveId = 1; slaveId <= maxSlaveId; slaveId++) {
      try {
        ...
        devices.push({
          deviceId: `modbus_rtu_slave_${slaveId}`,
        ...
        console.log(`[ModbusRTU] Found device at Slave ID ${slaveId}`);
      } catch (error) {
        // Device not responding, continue scanning
      }
    }

    console.log(`[ModbusRTU] Discovery complete. Found ${devices.length} devices.`);
    return devices;
  }
