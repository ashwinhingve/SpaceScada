...
Complete guide for deploying the WebSCADA MVP system to K3s.
...
- `-n, --namespace` - K3s namespace (default: webscada)
...
kubectl wait --for=condition=ready pod -l app.k3s.io/name=postgresql -n webscada --timeout=300s
kubectl wait --for=condition=ready pod -l app.k3s.io/name=redis -n webscada --timeout=300s

# 6. Deploy ConfigMaps and Secrets
kubectl apply -f infrastructure/k3s/manifests/base/ -n webscada

# 7. Deploy application components
kubectl apply -f infrastructure/k3s/manifests/backend-deployment.yaml -n webscada
kubectl apply -f infrastructure/k3s/manifests/frontend-deployment.yaml -n webscada
kubectl apply -f infrastructure/k3s/manifests/ingress.yaml -n webscada
...
kubectl apply -f https://raw.githubusercontent.com/k3s/ingress-nginx/controller-v1.9.4/deploy/static/provider/cloud/deploy.yaml
...
kubeseal -f infrastructure/k3s/manifests/base/backend-secrets.yaml \
  -w infrastructure/k3s/manifests/base/backend-sealed-secrets.yaml

# Apply sealed secret
kubectl apply -f infrastructure/k3s/manifests/base/backend-sealed-secrets.yaml -n webscada
```

### 3. Update Configuration for Production

Edit `infrastructure/k3s/manifests/base/backend-config.yaml`:
...
Edit `infrastructure/k3s/manifests/ingress.yaml`:
...
kubectl apply -f infrastructure/k3s/manifests/monitoring/servicemonitor.yaml -n webscada
```
...
3. Upload `infrastructure/k3s/manifests/monitoring/grafana-dashboard.json`
4. Select Prometheus datasource
5. Click Import

### Configure Alerts

Alerts are configured in `infrastructure/k3s/manifests/monitoring/servicemonitor.yaml`:
...
kubectl get pods -l app.k3s.io/name=postgresql -n webscada

# Check PostgreSQL logs
kubectl logs -l app.k3s.io/name=postgresql -n webscada
...
kubectl logs -n ingress-nginx -l app.k3s.io/name=ingress-nginx
...
# Manual cleanup
kubectl delete namespace webscada-dev
...
3. Review documentation in `infrastructure/k3s/manifests/README.md`
