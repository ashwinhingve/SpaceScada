...
export async function bluetoothRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
    '/',
    async (request: FastifyRequest<{ Body: CreateBluetoothDeviceBody }>, reply: FastifyReply) => {
      try {
  ...
  fastify.put(
    '/:id',
    async (request: FastifyRequest<{ Body: UpdateBluetoothDeviceBody }>, reply: FastifyReply) => {
      try {
        const { id } = request.params as { id: string };
