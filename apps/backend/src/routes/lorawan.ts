...
export async function lorawanRoutes(fastify: FastifyInstance) {
   ...
   */
  fastify.get('/api/lorawan/applications/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
   ...
   * Create application
   */
  fastify.post('/api/lorawan/applications', async (request: FastifyRequest<{ Body: CreateApplicationBody }>, reply: FastifyReply) => {
    try {
   ...
   * Update application
   */
  fastify.put('/api/lorawan/applications/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { id } = request.params as { id: string };
   ...
   * Delete application
   */
  fastify.delete('/api/lorawan/applications/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { id } = request.params as { id: string };
   ...
   * Create gateway
   */
  fastify.post('/api/lorawan/gateways', async (request: FastifyRequest<{ Body: CreateGatewayBody }>, reply: FastifyReply) => {
    try {
   ...
   * Delete gateway
   */
  fastify.delete('/api/lorawan/gateways/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { id } = request.params as { id: string };
   ...
   */
  fastify.get('/api/lorawan/gateways/:id/stats', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { id } = request.params as { id: string };
   ...
   */
  fastify.get('/api/lorawan/applications/:applicationId/devices', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
   ...
   */
  fastify.get('/api/lorawan/devices/:devEui', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
   ...
   * Create device
   */
  fastify.post('/api/lorawan/devices', async (request: FastifyRequest<{ Body: CreateDeviceBody }>, reply: FastifyReply) => {
    try {
      const { activationMode, appKey, nwkKey, devAddr, appSKey, nwkSEncKey, ...deviceData } = request.body;
   ...
   * Update device
   */
  fastify.put('/api/lorawan/devices/:devEui', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
   ...
   * Delete device
   */
  fastify.delete('/api/lorawan/devices/:devEui', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
   ...
   */
  fastify.post('/api/lorawan/devices/:devEui/downlink', async (request: FastifyRequest<{ Body: SendDownlinkBody }>, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
   ...
   * Get device metrics
   */
  fastify.get('/api/lorawan/devices/:devEui/metrics', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
   ...
   */
  fastify.get('/api/lorawan/devices/:devEui/telemetry', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
      const {
        metric_name,
   ...
   */
  fastify.get('/api/lorawan/devices/:devEui/telemetry/latest', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
   ...
   */
  fastify.post('/api/lorawan/devices/:devEui/telemetry', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
   ...
   */
  fastify.get('/api/lorawan/devices/:devEui/telemetry/stats', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
   ...
   */
  fastify.get('/api/lorawan/devices/:devEui/logs', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
      const {
        log_level,
   ...
   */
  fastify.get('/api/lorawan/devices/:devEui/logs/errors', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
   ...
   */
  fastify.get('/api/lorawan/devices/:devEui/logs/recent', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
      const { limit } = request.query as any;
   ...
   */
  fastify.post('/api/lorawan/devices/:devEui/logs', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
   ...
   */
  fastify.get('/api/lorawan/devices/:devEui/logs/stats', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
   ...
   */
  fastify.get('/api/lorawan/devices/:devEui/configuration', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
   ...
   */
  fastify.post('/api/lorawan/devices/:devEui/configuration', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
   ...
   */
  fastify.get('/api/lorawan/devices/:devEui/configuration/history', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { devEui } = request.params as { devEui: string };
      const { limit } = request.query as any;

      const history = await logsService.getConfigurationHistory(devEui, limit ? parseInt(limit) : 50);
